...
cc_cmd = IO.pipe do |r, w|
  puts "crystal build --release -o bin/segokaro-darwin-x86_64 --cross-compile --target #{clang_target} bin/segokaro.cr"
  Process.run "crystal", ["build", "--release", "-o", "bin/segokaro-darwin-x86_64", "--cross-compile", "--target", clang_target, "bin/segokaro.cr"], output: w
  w.close
  r.gets_to_end
end
